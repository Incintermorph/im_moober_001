 ...
 ******************************************************** */
	function fn_egov_selectGuestForupdt(nttId) {
		document.articleForm.nttId.value = nttId;	
		document.articleForm.action = "<c:url value='/mng/cop/bbs/updateGuestArticleView.do'/>";
		document.articleForm.submit();
	}
...
function fn_egov_select_guestList(pageNo) {
	...
	document.articleForm.action = "<c:url value='/mng/cop/bbs/selectGuestArticleList.do'/>";
	document.articleForm.submit();
}
</script>
<!-- 댓글 작성 스크립트  -->
...
<!-- 방명록 리스트 출력 -->
<form name="articleForm" action="<c:url value='/mng/cop/bbs/updateGuestArticleView.do'/>" method="post">
<div class="board">
<h2>${pageTitle}</h2>
<div class="reply">
	<ul>
...
<!-- 방명록 입력폼 -->
<form:form commandName="articleVO" action="${pageContext.request.contextPath}/cop/bbs/insertGuestArticle.do" method="post" onSubmit="fn_egov_insert_guest(document.forms[2]); return false; " style="float:left; clear:both;">
	<div class="wTableFrm" >
				...
				<td class="nopd">
					...
					<c:choose>
						<c:when test="${articleVO.nttId == '0'}">
							<span style="float:left;"><a href="javascript:fn_egov_insert_guest(document.forms[2]); " class="btn_s re_btn" title="<spring:message code="button.comment" /> <spring:message code="input.button" />"><spring:message code="button.comment" /><spring:message code="button.create" /></a></span>
						</c:when>
						<c:otherwise>
							<span style="float:left;"><a href="javascript:fn_egov_updt_guest(document.forms[2]); " class="btn_s re_btn"title="<spring:message code="button.update" /> <spring:message code="input.button" />"><spring:message code="button.comment" /><spring:message code="button.update" /></a></span>
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
