...
var REQ = {
	...
	init : function(){
		...
		this.req.save.cfg.url    =  "<c:url value="/user/objcAply/update.do"/>";
...
<table class="tbl_row">
	...
	<tbody>
			...
			<th scope="row">양성기관<span class="c_red">*</span></th>
				<td>
					<select name="agncyId" >
						...
						<c:if test="${row.agncy.agncyId eq  iMObjcAply.agncyId}">selected</c:if>
			...
			<tr>
				...
				<td><im:cd type="radio" codeId="IM0001" name="crsGrdCdv" selectedCode="${detail.objcAplyDtl.crsGrdCdv}"/></td>
			</tr>
			
			<c:if test="${iMObjcAply.dvsnCdv eq '01'}">
			<tr>
				<th scope="row">과정구분<span class="c_red">*</span></th>
				<td><im:cd type="radio" codeId="IM0002" name="crsDvsnCdv" selectedCode="${detail.objcAplyDtl.crsDvsnCdv}"/></td>
			</tr>
			</c:if>
			<c:if test="${iMObjcAply.dvsnCdv eq '01' || iMObjcAply.dvsnCdv eq '02'}">
			<tr>
				...
				<option value="${num}"
				<c:if test="${num eq  detail.objcAplyDtl.eduYear}">selected</c:if>
				>${num}</option>
				</c:forEach>
				</td>
			</tr>
			<tr>
				...
				<option value="${num}"
				<c:if test="${num eq  detail.objcAplyDtl.eduRnd}">selected</c:if>
				>${num}</option>
				</c:forEach>
				</select>
				</td>
			...
			<tr>
				...
				<option value="">선택</option>
				<im:cd type="option" codeId="${objcDvsnCdvArr}" selectedCode="${iMObjcAply.objcDvsnCdv}"/>
				</select>
				</td>
			</tr>
			<tr>
				...
				<td>
				<form:textarea path="aplyDesc" title="내용"/>
				</td>
			</tr>
		...
		<div class="b_box">
			<a href="javascript:;"  onclick="REQ.save()" class="c_btn d_green mid">저장</a>
			<a href="javascript:;"  onclick="REQ.detail({'objcAplyId' : '${iMObjcAply.objcAplyId}'});" class="c_btn wt mid">취소</a>
		</div>