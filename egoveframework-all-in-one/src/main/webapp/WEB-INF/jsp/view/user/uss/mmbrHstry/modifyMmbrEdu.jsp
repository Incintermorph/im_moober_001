...
var AJAX = {
	...
	initReq: function(){
		...
		this.req.save.cfg.url    =  "<c:url value="/user/mmbrEdu/update.do"/>";
		...
		AJAX.validator();	
		 ...
		 this.req.del = imRequest("ajax");
		    this.req.del.cfg.formId = "iMMmbrEdu";
		    this.req.del.cfg.type   = "json";
		    this.req.del.cfg.url    = "<c:url value="/user/mmbrEdu/delete.do"/>";
	...
	this.req.save.validator.set({
	 ...
	 this.req.save.validator.set(function(){
		 var form =  document.getElementById(AJAX.req.save.cfg.formId);
    	 if(form.elements["dgeFileId"].value==''){
	    	if(!COMMT.chkAttachFile('${ATCH_FILE_ID}')){
	...
	cancle : function(){
		REQ.unloadCancel('iMMmbrEdu');
				...
				<input type="hidden" name="mmbrEduId" value="${iMMmbrEdu.mmbrEduId}"/>
					...
					<table class="tbl_row al">
						...
						<tbody>
							<tr>
								<th scope="row">학교명 *</th>
								<td>
									<input type="text" name="sclNm" maxlength="20" class="wide" value="${iMMmbrEdu.sclNm}"  title="학교명">
								</td>
							</tr>
							<tr>
								<th scope="row">학과 *</th>
								<td>
									<input type="text"  name="mjrNm" maxlength="20" value="${iMMmbrEdu.mjrNm}"   class="wide" title="학과">
								</td>
							</tr>
							<tr>
								<th scope="row">전공 *</th>
								<td>
									<input type="text" class="wide" value="${iMMmbrEdu.specNm}"  maxlength="20" name="specNm" title="전공">
								</td>
							</tr>
							<tr>
								<th scope="row">학위 *</th>
								<td>
									<im:cd codeId="${dgeCdvArr}" type="radio" selectedCode="${iMMmbrEdu.dgeCdv}"  name="dgeCdv"/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<div class="c_date">
										<input type="text" value="<im:dt yyyyMMddHHmmss="${iMMmbrEdu.dgeYmd}"/>" title="학위취득일" data-role="datepicker" name="dgeYmd">
									</div>
								</td>
							</tr>
							<tr>
								...
								<td>
									...
									<input type="hidden" name="dgeFileId"  value="${iMMmbrEdu.dgeFileId}"/>
							...
							<tr>
								...
								<div class="ct_box">
									...
									<textarea  name="rechTpc" id="rechTpc"  cols="" rows="" style="height:100px;">${iMMmbrEdu.rechTpc}</textarea>
								</td>
							</tr>
						</tbody>
