...
var AJAX = {
	...
	initReq: function(){
		...
		this.req.save.cfg.fn.complete = function(act, data) {
		 ...
		 this.req.del = imRequest("ajax");
		    this.req.del.cfg.formId = "iMMmbrWorkHstry";
		    this.req.del.cfg.type   = "json";
		    this.req.del.cfg.url    = "<c:url value="/user/mmbrWorkHstry/delete.do"/>";
	...
	validator : function(){
		 ...
		 this.req.save.validator.set(function(){
	    	 ...
	    	 if(form.elements["evddocFileId"].value==''){
		 ...
		 this.req.save.validator.set(function(){
			 var form =  document.getElementById(AJAX.req.save.cfg.formId);
	    	 if(form.elements["dsgnFileId"].value==''){
		    	if(!COMMT.chkAttachFile('${ATCH_FILE_ID2}')){
	...
	delPotalAgncy: function(){
		...
		form.elements["agncyCode"].value="";
				...
				<input type="hidden" name="mmbrWorkHstryId" value="${param['mmbrWorkHstryId']}"/>
					<table class="tbl_row al">
						...
						<tbody>
							<tr>
								<th scope="row">구분 *</th>
								<td>
									<im:cd codeId="${workDvsnArr}" type="radio" onclick="AJAX.esntl(this.value)" name="workDvsnHstryCdv" selectedCode="${iMMmbrWorkHstry.workDvsnHstryCdv}"/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<div class="search_area">
										<input type="text" name="agncyNm"  value="${iMMmbrWorkHstry.agncyNm}" placeholder="재직기관을 선택하여 주세요" title="기관명" readonly="readonly" >
										<input type="hidden" name="agncyCode"  value="${iMMmbrWorkHstry.agncyCode}" placeholder="기관코드을 선택하여 주세요" title="기관코드">
									...
									<input type="checkbox" <c:if test="${iMMmbrWorkHstry.eduDsgnYn eq 'Y'}">checked</c:if> name="eduDsgnYn"  value="Y"  id="eduDsgnYn_">
							...
							<tr>
								...
								<td>
									<div class="c_date">
										<input type="text" name="workBgnYmd" value="<im:dt yyyyMMddHHmmss="${iMMmbrWorkHstry.workBgnYmd}"/>" placeholder="시작일" title="시작일" data-role="datepicker">
									</div>
									~
									<div class="c_date">
										<input type="text"  name="workEndYmd" value="<im:dt yyyyMMddHHmmss="${iMMmbrWorkHstry.workEndYmd}"/>"  placeholder="종료일" title="종료일" data-role="datepicker">
									</div>
									<div class="vspace"></div>
									<input type="checkbox" name="workYn" value="Y"
									<c:if test="${iMMmbrWorkHstry.workYn  eq 'Y'}">checked</c:if>
									  id="workYn_">
									<label for="workYn_">현재 근무 중</label>
								</td>
							</tr>
							<tr>
								<th scope="row">직급 <span class="esntlStar">*</span></th>
								<td>
									<input type="text" class="wide" value="${iMMmbrWorkHstry.pstn}" title="직급" maxlength="20" name="pstn">
								</td>
							</tr>
							<tr>
								...
								<td>
									<select name="workTrgtCdv" id="">
										<im:cd codeId="${workTrgtCdvArr}" type="option"  selectedCode="${iMMmbrWorkHstry.workTrgtCdv}"/>
									</select>
									<div class="vspace"></div>
									<input type="text" name="etcWork" class="wide" value="${iMMmbrWorkHstry.etcWork}" placeholder="기타 대상업무를 입력하세요" title="기타">
								</td>
							</tr>
							<tr>
								<th scope="row">지정서 첨부 <span class="esntlStar">*</span></th>
								<td>
									...
									<input type="hidden" value="${iMMmbrWorkHstry.evddocFileId}"  name="evddocFileId" />
							...
							<tr>
								...
								<td>
									...
									<input type="hidden" name="dsgnFileId" value="${iMMmbrWorkHstry.dsgnFileId}"  />
