...
var REQ = {
	...
	search : function(){
		...
		form.elements["aplcnt.scExamFnshYn"].value= serchform.elements["aplcnt.scExamFnshYn"].value;
		REQ.req.list.go();
	},
	...
	modify : function(wtstAplcntId){ 
		
		var req = imRequest();
		req.cfg.formId = "FormPageDetailTab";
		req.cfg.url     = "<c:url value="/mng/wtst/wtstAplcnt/modify.do"/>";
		
		var form =  document.getElementById(req.cfg.formId);
		form.elements["wtstAplcntId"].value= wtstAplcntId;
		req.go();
	},
	bulkMarking:function(){
		var req = imRequest("layer");
		req.cfg.url    = "<c:url value="/mng/wtstAplcnt/bulkMarkingExcelLayer.do"/>";
		req.cfg.options.title="시험결과 일괄등록";
		req.cfg.formId = "FormPageDetail";
...
<c:set var="scKey">mberId=회원아이디,mberNm=이름,eml=이메일,mblTelno=휴대전화번호,wtstAplcntId=원서접수아이디</c:set>
	<div class="sch_box">
		<div class="group">
			...
			<select name="aplcnt.scExamFnshYn" onchange="REQ.search()" >
				<option value="">시험결과</option>
				<im:cd type="option" codeId="${ARR_IM0025}"  selectedCode="${condition.aplcnt.scExamFnshYn}"/>
			</select>	
		</div>
		<div class="group">
...
<div class="cb_bar">
	...
	<div>
	 <select id="sttsCdv_">
	 <option value="">시험결과</option>
	 <im:cd type="option" codeId="${ARR_IM0025}" except="02,03"/>
	 </select>
	 <a href="#"  onclick="REQ.updateAply('FNSH')"  class="bt_new bk">변경</a>
	
	 <a href="#" onclick="REQ.bulkMarking();return false;" class="bt_new bk">시험결과 일괄등록</a>
...
<table class="tbl_col">
	...
	<tbody>
		...
		<tr>
			...
			<a href="javascript:;" onclick="REQ.modify('${row.wtstAplcnt.wtstAplcntId}')" >${row.wtstAplcnt.tktstno}</a>
			</c:if>
			</td>
			<td><c:out value="${row.wtstAplcnt.avgScr}"/></td>
			<td><c:out value="${row.wtstAplcnt.fltpSbjCnt}"/></td>
			<td>
			<c:set var="FNSH">
